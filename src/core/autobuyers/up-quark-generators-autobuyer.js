...
export class UpQuarkGeneratorAutobuyerState extends UpgradeableAutobuyerStateQuantum {
  get tier() {
    return this.id;
  }

  get name() {
    return UpQuarkGenerator(this.tier).shortDisplayName;
  }
  ...
  get data() {
    return player.auto.upQuarkGens.all[this.tier - 1];
  }

  get baseInterval() {
    return Player.defaultStart.auto.upQuarkGens.all[this.tier - 1].interval;
  }
  ...
  get canBeUpgraded() {
    return FusionChallenge((this.tier * 2) - 1).isCompleted;
  }
  ...
  get canTick() {
    ...
    const settingConfig = player.auto.upQuarkGens;
    const individualSetting = settingConfig.all[this.tier - 1];
    const groupSetting = settingConfig.isActive;
    const thisSetting = individualSetting && (Autobuyer.upQuarkGenerator.collapseDisplay ? groupSetting : true);

    // General availability
    const dim = UpQuarkGenerator(this.tier);
  ...
  tick() {
    ...
    switch (this.mode) {
      case AUTOBUYER_MODE.BUY_SINGLE:
        buySingleUpQuarkGenerator(tier);
        break;
      case AUTOBUYER_MODE.BUY_MAX:
        buyMaxUpQuarkGenerator(tier, this.bulk);
        break;
    }
  }
  ...
  static get autobuyerGroupName() { return "Up-type"; }
  static get isActive() { return player.auto.upQuarkGens.isActive; }
  static set isActive(value) { player.auto.upQuarkGens.isActive = value; }
