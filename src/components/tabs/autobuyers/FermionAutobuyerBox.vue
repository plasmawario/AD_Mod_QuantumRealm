...
export default {
  ...
  computed: {
    ...
    isShowingStateInfo() {
      ...
      return this.isActive && ["Fusion", "Eternity"].includes(this.autobuyer.name) && this.currMode !== 0;
    },
    extraInfo() {
...
<template>
  ...
  >
    {{ name }}
    <br>
    Requirement: {{ format(cost) }} Total Quarks this fusion
  </div>
</template>
